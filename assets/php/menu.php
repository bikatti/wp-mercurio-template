                ...
                <?php 
                    ...
                    foreach($categories as $category) {
                        ...
                        if(!$category->parent && $catMenu == 1 && $close <= 8) :
                            ...
                            echo '<a class="m-menuOutNav__expander" href="' . get_category_link($category->term_id) . '" data-ripple><span class="a-icon a-icon__expander"></span></a>';
                                echo '<ul class="m-menuOutNav__submenu" data-collapsible-panel data-collapsible-breakpoint="mobile-only">';

                                $taxonomy_name = 'category';
                                $termChildren = get_term_children( $category->term_id, $taxonomy_name );
                                    foreach($termChildren as $child) {
                                        $catChild = get_term_by( 'id', $child, $taxonomy_name );
                                        echo '<li class="m-menuOutNav__item m-menuOutNav__itemSub"><a class="m-menuOutNav__link m-menuOutNav__linkSub -semiBold" href="' . get_term_link( $child, $taxonomy_name ) . '"><p class="-colorLightin">' . $catChild->name . '<p></a></li>';
                                    }
                                echo '</ul>';
                            echo '</li>';
                        endif;
